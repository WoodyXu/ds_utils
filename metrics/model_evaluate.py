...
def model_evaluate(truth, pred):
    ...
    agg2 = (agg1.sort_index(by='min_pred')).reset_index(drop=True)
    agg2["odds"] = agg2["pos_num"] * 1.0 / agg2["neg_num"]
    agg2["neg_rate"] = agg2["neg_num"] * 1.0 / agg2["total"]
    agg2["ks"] = np.abs((agg2["pos_num"] * 1.0 / agg2["pos_num"].sum()).cumsum() - \
            (agg2["neg_num"] * 1.0 / agg2["neg_num"].sum()).cumsum()) * 100
    ...
    accuracy = (tp + tn) * 1.0 / (tp + fp + fn + tn + 0.000001)
    precision = tp * 1.0 / (tp + fp + 0.000001)
    recall = tp * 1.0 / (tp + fn + 0.000001)

    return auc, ks, opt_cut, accuracy, precision, recall

if __name__ == "__main__":
    preds = np.random.random(1000)
    truths = np.random.randint(2, size=1000)

    print model_evaluate(truths, preds)
    print predict_evaluate.evaluate(truths, preds)
